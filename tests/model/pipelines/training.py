...
class FuseMyCellTraining(FlowSpec, DatasetMixin):
    ...
    def start(self):
        ...
        if hasattr(self, 'test_mode') and self.test_mode:
            ...
            dummy_data = torch.randn(1, 1, 64, 128, 128)  # [B, C, Z, Y, X]
            dummy_target = torch.randn(1, 1, 64, 128, 128)
            dummy_dataset = TensorDataset(dummy_data, dummy_target)
            
            self.train_loader = DataLoader(dummy_dataset, batch_size=self.training_batch_size)
            self.val_loader = DataLoader(dummy_dataset, batch_size=self.training_batch_size)
        else:
            # Load real dataset
            try:
